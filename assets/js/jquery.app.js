...
function($, window) {
  ...
  sideNav.init = function() {
   /* $(".side-nav .side-nav-menu li a").on("click", function(e) {
      ...
      $(".app").toggleClass("side-nav-folded"), e.preventDefault()
      ...
      $(".app").toggleClass("side-nav-expand"), e.stopPropagation(), $(".side-nav-backdrop").on("click", function(e) {
        $(".app").removeClass("side-nav-expand"), $(this).remove()
      })
    })*/
...
function($) {
  sideNav.init()
}(jQuery), $(document).on("change", 'input[name="header-theme"]', function() {
